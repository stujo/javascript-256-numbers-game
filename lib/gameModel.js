function GameModel(startingState) {
    ...
    this._history = [this.wrapStateForHistory(state)]
}

GameModel.prototype.wrapStateForHistory = function(state) {
    return {
        board: state,
...
GameModel.prototype.emptyIndexes = function(state) {
    var candidates = []
...
GameModel.prototype.spawnValue = function() {
    return Math.random() > 0.9 ? 4 : 2;
}

GameModel.prototype.spawnLocation = function(gaps) {
    return gaps[Math.floor(Math.random() * gaps.length)];
}

GameModel.prototype.spawn = function(new_state) {
    var state = new_state.slice(0);
    var gaps = this.emptyIndexes(state)

    if (gaps.length > 0) {
        state[this.spawnLocation(gaps)] = this.spawnValue()
    }

    return state;
}

GameModel.prototype.boardsEqual = function(b1, b2) {
...
GameModel.prototype.move = function(move_name) {
    ...
    status.board = this[`${move_name}_impl`](start_board);

    if (!this.boardsEqual(status.board, start_board)) {
        var gaps = this.emptyIndexes(status.board);

        if (gaps.length > 0) {
            status.spawn_location = this.spawnLocation(gaps);
            status.spawn_value = this.spawnValue();
